...
public class GPUImage {
    ...
    public GPUImage(final Context context) {
        if (!supportsOpenGLES2(context)) {
            throw new IllegalStateException("OpenGL ES 2.0不被支持");
        }
        mContext = context;
        mFilter = new GPUImageFilter();
        mRenderer = new GPUImageRenderer(mFilter);//将滤镜器加载到渲染器
    }
    ...
    /**
     * GLSurfaceView的简单配置和设置Render
     *
     * @param view 需要配置的GLSurfaceView
     * @return 空
     */
    public void setGLSurfaceView(final GLSurfaceView view) {
        mGlSurfaceView = view;
        // 创建一个OpenGL ES 2.0 context来使用 2.0 API
    ...
    /**
     * 开启camera预览，将预览回调获取的视频帧给Render渲染
     *
     * @param camera         要预览的camera
     * @param degrees        摄像头的方向
     * @param flipHorizontal 用于判断是否需要调整camera
     * @param flipVertical
     * @return: 空
     */
    public void setUpCamera(final Camera camera, final int degrees, final boolean flipHorizontal,
        ...
        if (Build.VERSION.SDK_INT > Build.VERSION_CODES.GINGERBREAD_MR1) {
            //视频流先渲染到Surfacetexture，Surfacetexture再显示到GlSurfaceView，实质上并没有对surfacetexture再利用
            setUpCameraGingerbread(camera);
        } else {
            //直接在获取视频流的回调方法里获取纹理去渲染
    ...
    //以SurfaceTexture来预处理camera的帧，再显示在设备上
    @TargetApi(11)
...
//    }
     ...
     * @param fileName   处理后要保存的文件名
     * @param listener   保存图片的监听
     * @return: 空
     */
