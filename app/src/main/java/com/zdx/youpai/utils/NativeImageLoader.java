...
public class NativeImageLoader {
    private final String TAG = "NativeImageLoader";
    ...
    public Bitmap loadNativeImage(final String path, final Point mPoint, final NativeImageCallBack mCallBack){
        ...
        //若该Bitmap不在内存缓存中，则启用线程池去加载本地的图片，并将Bitmap加入到mMemoryCache中
        if(bitmap == null){
    ...
    private Bitmap decodeThumbBitmapForFile(String path, int viewWidth, int viewHeight){
        ...
        //设置为true,表示解析Bitmap对象，该对象不占内存只获取图片的宽高数据
...
//        Log.d(TAG, "缩略图宽"+String.valueOf(options.outWidth));
//        Log.d(TAG, "缩略图高"+String.valueOf(options.outHeight));

        //设置为false,解析Bitmap对象加入到内存中,这样就完成了裁剪
...
//        Log.d(TAG, "图片宽"+bitmapWidth);
//        Log.d(TAG, "图片高"+bitmapHeight);

        //假如Bitmap的宽度或高度大于我们自定义View的宽高，则计算缩放比例
...
//        Log.v(TAG, "图片缩放比例"+String.valueOf(inSampleSize));
